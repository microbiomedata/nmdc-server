...
onMounted(async () => {
  ...
  if (r && schema) {
    ...
    // Fetch suggestions generated by study-level forms that are relevant to the active template
    // TODO: this might need to be gated on changes that actually influence the suggestions. How much effort
    //       we invest in that might be guided by how long the suggestion generation takes.
    metadataSuggestions.value.push(...await getMetadataSuggestionsFromStudyDetails(props.id));
