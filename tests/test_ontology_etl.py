...
def test_populate_envo_from_generic_ontology(db: Session):
    ...
    direct_ancestor = db.query(models.EnvoAncestor).filter_by(
        id="ENVO:00000002",
        ancestor_id="ENVO:00000001",
        direct=True
    ).first()
    assert direct_ancestor is not None
    
    # Check closure relationship
    closure_ancestor = db.query(models.EnvoAncestor).filter_by(
        id="ENVO:00000002",
        ancestor_id="ENVO:00000001",
        direct=False
    ).first()
...
def test_ontology_etl_integration(db: Session):
    ...
    ontology.load(
        db,
        MockCursor(class_data),
        MockCursor(relation_data)
    )
