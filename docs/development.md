...
3. Set the following configuration in `.env`.
   ...
   ```
4. Populate the below fields in `.env`. Values for these should be generated by running `openssl rand -hex 32`. You will have to run the command twice total to get a value for each field.
   ```bash
...
1. Ensure that you have completed the sections above of configuring [NERSC credentials](#nersc-credentials) and [MongoDB credentials](#mongodb-credentials). 
