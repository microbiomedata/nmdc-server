...
def mongo_populate_envo_ancestor(
    db: MongoClient,
    term_id: str,
    node: str,
    edges: Dict[str, Set[str]],
    ...
    for parent in edges[node]:
        ...
        ancestor = dict(_id=dict(id=term_id, ancestor_id=parent), direct=direct)
        db.nmdc.envo_ancestor.update_one({"_id": ancestor["_id"]}, {"$set": ancestor}, upsert=True)

    for parent in edges[node]:
        if parent not in all_nodes:
            continue  # skip ancestors outside the simplified hierarchy

        mongo_populate_envo_ancestor(db, term_id, parent, edges, all_nodes, False, visited)


def mongo_get_biosample_roots(db: MongoClient) -> Dict[str, Set[str]]:
    """
    Find all reachable envo root terms from each biosample envo facet.
    ...
    """
    parents: Dict[str, str] = {}
    query = db.nmdc.envo_ancestor.find(dict(direct=True))
    for ancestor in query:
        parents[ancestor["_id"]["id"]] = ancestor["_id"]["ancestor_id"]

    def reachable_roots(attr: str) -> Set[str]:
        query = db.nmdc.biosample_set.distinct(attr)
        terms = set(r[0] for r in query) - {None}
        roots = set()
    ...
    return {
        key: reachable_roots(key)
        for key in ["env_broad_scale.term.id", "env_local_scale.term.id", "env_medium.term.id"]
    }


def _mongo_build_envo_subtree(db: MongoClient, parent_id: str) -> None:
    db.nmdc.envo_ancestor.find({"_id.ancestor_id": parent_id, "direct": True})
    for node in query:
        db.nmdc.envo_tree.insert_one(dict(_id=node["_id"]["id"], parent_id=parent_id))
        _mongo_build_envo_subtree(db, node.id)


def mongo_build_envo_trees(db: Session) -> None:
    """
    Convert the envo_ancestors graph into trees, and store them (normalized).
    ...
    This should only be called after biosamples have been ingested.
    """
    db.nmdc.envo_tree.drop()

    roots = mongo_get_biosample_roots(db)
    root_set = set(itertools.chain(*roots.values()))
    for root in root_set:
        db.nmdc.nmdc_tree.insert_one(
            {
                "_id": root,
                "parent_id": None,  # null parent_id indicates root node(s)
            }
        )
        _mongo_build_envo_subtree(db, root)

    mongo_nested_envo_trees.cache_clear()


def _mongo_get_trees_for_facet(
    db: MongoClient,
    facet: str,
    tree_nodes: Dict[str, _NodeInfo],
    tree_children: TreeChildren,
    ...
    This is a pure function.
    """
    query = db.nmdc.biosample_set.distinct(facet)
    present_terms = set(r[0] for r in query) - {None}
    reachable: Set[str] = set()
...
@functools.lru_cache(maxsize=None)
def mongo_nested_envo_trees() -> Dict[str, List[EnvoTreeNode]]:
    tree_children = defaultdict(list)
    tree_nodes: Dict[str, _NodeInfo] = {}
...
def mongo_load(db: MongoClient):
    db.nmdc.envo_term.drop()
    db.nmdc.envo_ancestor.drop()
    ...
    for graph in envo_data["graphs"]:
        ...
        for node in graph["nodes"]:
            ...
            label = node.pop("lbl", "")
            data = node.get("meta", {})
            envo_data = dict(_id=id, label=label, data=data)
            db.nmdc.envo_term.insert_one(envo_data)
            ids.add(id)

        for node in ids:
            ancestor_data = dict(_id=dict(id=node, ancestor_id=node), direct=False)
            db.nmdc.envo_ancestor.insert_one(ancestor_data)
            mongo_populate_envo_ancestor(db, node, node, direct_ancestors, ids, True, set())
