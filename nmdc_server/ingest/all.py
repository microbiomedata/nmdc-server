...
def load(db: Session, function_limit=None, skip_annotation=False):
    ...
    if skip_annotation is False:
        ...
        # hours to ingest all of the gene function products from the metag/metat
        # workflows.
        # try:
        ...
        #         )
        # except Exception:
        #     logger.exception("Failed during metagenome ingest.")
        # finally:
        #     db.commit()

        try:
            logger.info("Loading metatranscriptome annotation...")
            count = mongodb["metatranscriptome_activity_set"].estimated_document_count()
            iterator = paginate_cursor(
                mongodb["metatranscriptome_activity_set"],
                page_size=1,  # prevent cursor from timing out
                no_cursor_timeout=True,
            )
            with click.progressbar(iterator, length=count) as bar:
            ...
            logger.exception("Failed during metatranscriptome ingest.")
        finally:
            db.commit()
