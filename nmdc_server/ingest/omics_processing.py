...
def get_poolable_replicate_manifest(omics_processing, data_object_id, mongodb: Database) -> str:
    """
    ...
    manifest_document = mongodb["manifest_set"].find_one({"id": data_object_id})
...
def load_omics_processing(db: Session, obj: Dict[str, Any], mongodb: Database, logger, config_map):
    ...
    manifest_id = omics_processing.id
    for data_object_id in data_objects:
        ...
        manifest_id: str = get_poolable_replicate_manifest(
            omics_processing, data_object_id, mongodb
        )
