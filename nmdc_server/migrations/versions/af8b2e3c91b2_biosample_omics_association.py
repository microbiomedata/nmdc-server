...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['biosample_id'], ['biosample.id'], name=op.f('fk_biosample_input_association_biosample_id_biosample')),
        sa.ForeignKeyConstraint(['omics_processing_id'], ['omics_processing.id'], name=op.f('fk_biosample_input_association_omics_processing_id_omics_processing')),
        sa.PrimaryKeyConstraint('omics_processing_id', 'biosample_id', name=op.f('pk_biosample_input_association'))
    )
    ...
    for omics_processing in session.query(OmicsProcessing):
        if omics_processing.biosample_id:
            mappings.append({"biosample_id": omics_processing.biosample_id, "omics_processing_id": omics_processing.id})
    op.bulk_insert(biosample_input_association, mappings)



    op.drop_constraint('fk_omics_processing_biosample_id_biosample', 'omics_processing', type_='foreignkey')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('omics_processing', sa.Column('biosample_id', sa.VARCHAR(), autoincrement=False, nullable=True))
    op.create_foreign_key('fk_omics_processing_biosample_id_biosample', 'omics_processing', 'biosample', ['biosample_id'], ['id'])
